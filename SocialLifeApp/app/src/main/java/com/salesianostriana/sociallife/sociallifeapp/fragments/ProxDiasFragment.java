...
public class ProxDiasFragment extends Fragment {

    RecyclerView recyclerView;
    public ProxDiasFragment() {
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_prox_dias, container, false);
        recyclerView = (RecyclerView) v.findViewById(R.id.recyclerViewPlanesProximosDias);
        ...
        if(ubicacion==null){
            ...
            new ObtenerPlanesProximosDias().execute(Preferencias.preferencias.getString("token",null), ubicacion, Utils.fechaHoyDjango());
        }

        return v;
    }

    private class ObtenerPlanesProximosDias extends AsyncTask<String, Void, List<PlanCompleto>> {

        @Override
        protected List<PlanCompleto> doInBackground(String... params) {
            Call<List<PlanCompleto>> call = Servicio.instanciarServicio(PlanesApi.class, params[0]).obtenerPlanesProximosDias();
    ...
    private class ObtenerMisDatos extends AsyncTask<Void, Void, List<PerfilUsuario>>{
        ...
        protected void onPostExecute(List<PerfilUsuario> lista) {
            ...
            if(lista.size()!=0){
                ...
                new ObtenerPlanesProximosDias().execute(Preferencias.preferencias.getString("token", null), ub_actual, Utils.fechaHoyDjango());
            }
        }
    }
